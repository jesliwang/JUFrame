...
#if FEAT_COMPILER
        protected override void EmitWrite(MuffinProtoBuf.Compiler.CompilerContext ctx, MuffinProtoBuf.Compiler.Local valueFrom)
        {
            // int i and T[] arr
            using (Compiler.Local arr = ctx.GetLocalWithValue(arrayType, valueFrom))
            using (Compiler.Local i = new MuffinProtoBuf.Compiler.Local(ctx, ctx.MapType(typeof(int))))
            {
...
#if FEAT_COMPILER
        protected override void EmitRead(MuffinProtoBuf.Compiler.CompilerContext ctx, MuffinProtoBuf.Compiler.Local valueFrom)
        {
            Type listType;
#if NO_GENERICS
            ...
            using (Compiler.Local list = new Compiler.Local(ctx, listType))
            {
                ...
                using(Compiler.Local oldLen = AppendToCollection ? new MuffinProtoBuf.Compiler.Local(ctx, ctx.MapType(typeof(int))) : null) {
