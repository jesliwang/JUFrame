...
using System.Text;
using MuffinProtoBuf.Serializers;
...
namespace MuffinProtoBuf.Meta
{
    ...
    public class MetaType : ISerializerProxy
    {
        ...
        private IProtoTypeSerializer BuildSerializer()
        {
            if (Helpers.IsEnum(type))
            {
                return new TagDecorator(MuffinProtoBuf.Serializer.ListItemTag, WireType.Variant, false, new EnumSerializer(type, GetEnumMap()));
            }
            Type itemType = IgnoreListHandling ? null : TypeModel.GetListItemType(model, type);
            if (itemType != null)
            {
                ...
                ValueMember fakeMember = new ValueMember(model, MuffinProtoBuf.Serializer.ListItemTag, type, itemType, defaultType, DataFormat.Default);
                return new TypeSerializer(model, type, new int[] { MuffinProtoBuf.Serializer.ListItemTag }, new IProtoSerializer[] { fakeMember.Serializer }, null, true, true, null, constructType, factory);
            }
            if (surrogate != null)
            {
        ...
        internal static bool GetAsReferenceDefault(RuntimeTypeModel model, Type type)
        {
            ...
            {
                if (typeAttribs[i].AttributeType.FullName == "MuffinProtoBuf.ProtoContractAttribute")
                {
                    object tmp;
        ...
        internal void ApplyDefaultBehaviour()
        {
            ...
            for (int i = 0; i < typeAttribs.Length; i++)
            {
                ...
                if (!isEnum && fullAttributeTypeName == "MuffinProtoBuf.ProtoIncludeAttribute")
                {
                    int tag = 0;
                ...
                if (fullAttributeTypeName == "MuffinProtoBuf.ProtoPartialIgnoreAttribute")
                {
                ...
                if (!isEnum && fullAttributeTypeName == "MuffinProtoBuf.ProtoPartialMemberAttribute")
                {
                    if (partialMembers == null) partialMembers = new BasicList();
                    partialMembers.Add(item);
                }

                if (fullAttributeTypeName == "MuffinProtoBuf.ProtoContractAttribute")
                {
            ...
            foreach (MemberInfo member in foundList)
            {
                ...
                } else if ((method = member as MethodInfo) != null)
                {
                    ...
                    {
                        CheckForCallback(method, memberAttribs, "MuffinProtoBuf.ProtoBeforeSerializationAttribute", ref callbacks, 0);
                        CheckForCallback(method, memberAttribs, "MuffinProtoBuf.ProtoAfterSerializationAttribute", ref callbacks, 1);
                        CheckForCallback(method, memberAttribs, "MuffinProtoBuf.ProtoBeforeDeserializationAttribute", ref callbacks, 2);
                        CheckForCallback(method, memberAttribs, "MuffinProtoBuf.ProtoAfterDeserializationAttribute", ref callbacks, 3);
        ...
        internal static AttributeFamily GetContractFamily(RuntimeTypeModel model, Type type, AttributeMap[] attributes)
        {
            ...
            for (int i = 0; i < attributes.Length; i++)
            {
                ...
                {
                    case "MuffinProtoBuf.ProtoContractAttribute":
                        bool tmp = false;
        ...
        private static ProtoMemberAttribute NormalizeProtoMember(TypeModel model, MemberInfo member, AttributeFamily family, bool forced, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferByTagName)
        {
            ...
            if (isEnum)
            {
                attrib = GetAttribute(attribs, "MuffinProtoBuf.ProtoIgnoreAttribute");
                if (attrib != null)
                {
                    ignore = true;
                }
                ...
                {
                    attrib = GetAttribute(attribs, "MuffinProtoBuf.ProtoEnumAttribute");
            ...
            {
                attrib = GetAttribute(attribs, "MuffinProtoBuf.ProtoMemberAttribute");
                GetIgnore(ref ignore, attrib, attribs, "MuffinProtoBuf.ProtoIgnoreAttribute");
